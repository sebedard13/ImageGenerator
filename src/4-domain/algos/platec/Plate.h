...
public:
	...
	/// @param	mapHeight	Amount of crust to add.
	/// @param	age	Time of creation of new crust.
	void addCrustByCollision(size_t x, size_t y, float mapHeight, size_t age, size_t activeContinent) throw();
	...
	const std::vector<size_t>& getAge();
	const float* getMap();
	...
	unsigned selectCollisionSegment(size_t coll_x, size_t coll_y) throw();
	...
	/// @param	x	Offset on the global world map along X axis.
	/// @param	y	Offset on the global world map along Y axis.
	/// @param	newMass	Amount of crust at given location.
	/// @param	ageOfNewCrust	Time of creation of new crust.
	void setCrust(size_t x, size_t y, float newMass, size_t ageOfNewCrust) throw();
...
private:
	...
	std::vector<size_t> segmentMap{}; ///< Segment ID of each piece of continental crust.
	std::vector<segmentData> segments; ///< Details of each crust segment.
