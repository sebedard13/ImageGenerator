...
public:
    ColorInterpolate(const T &min, const T &max, std::vector<QRgb> &colors = defaultColors)
            : min(min),
            range(max - min),
...
public:
    ColorInterpolate(const int &min, const int &max, std::vector<QRgb> &colors = defaultColors)
            : min(min),
            range(max - min) {
